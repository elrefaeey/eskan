...
const pricePoints = [
  ...
  { year: "2023", price: 72000 },
  { year: "2024", price: 85000 },
  { year: "2025", price: 95000 },
  { year: "2026", price: 110000 },
];
...
const data = pricePoints.map((point, index) => ({
  ...
  growth:
    index === 0
      ? null
      : `+${Math.round(((point.price - pricePoints[index - 1].price) / pricePoints[index - 1].price) * 100)}%`,
}));

const totalGrowth = `+${Math.round(
  ((pricePoints[pricePoints.length - 1].price - pricePoints[0].price) / pricePoints[0].price) * 100,
)}%`;

const COLORS = ["#a8d5b5", "#8ecfaa", "#5cb888", "#3d9a6e", "#1F503B"];
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-l from-primary to-[#2d8a57] px-4 py-4 sm:px-6 sm:py-5 md:px-8" dir="rtl">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center gap-1.5 bg-white/15 rounded-xl px-3 py-2 border border-white/20 shrink-0 self-start sm:self-auto">
            ...
            <span className="text-white/70 text-xs whitespace-nowrap">نمو خلال 5 سنوات</span>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden px-0 pt-4 pb-1 sm:pt-6 md:px-4 md:pb-2 lg:px-6">
        <div
          className={cn(
            "h-[250px] sm:h-[270px] md:h-[300px]",
            isCompact ? "translate-x-0" : "-translate-x-1",
          )}
        >
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis
                hide={isCompact}
                domain={[55000, 115000]}
                tick={{ fill: "#aaa", fontSize: 9, dx: -3 }}
                axisLine={false}
                tickLine={false}
                tickMargin={0}
                tickFormatter={(v) => formatPrice(v)}
                ticks={[55000, 70000, 85000, 100000, 115000]}
                width={isCompact ? 0 : 42}
              />
      ...
      <div className="grid grid-cols-5 gap-px border-t border-gray-100 bg-gray-100" dir="ltr">
        {summaryRows.map((row, i) => {
          const isLatest = i === summaryRows.length - 1;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isLatest ? "bg-primary/5" : "bg-white",
              )}
            >
              <span
                className={cn(
                  "font-extrabold leading-none tabular-nums whitespace-nowrap",
                  "text-[9px] sm:text-sm md:text-lg",
                  isLatest ? "text-primary" : "text-gray-700",
                )}
              >

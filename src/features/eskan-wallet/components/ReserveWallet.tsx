...
import { useReserveWallet } from "../hooks/useReserveWallet";
...
const ReserveWallet = ({
  ...
  const { mutate: reserveUnit, isPending } = useReserveWallet();

  const availableShares =
    unitData.contracted_shares !== "null"
      ? unitData.shares_num - +unitData.contracted_shares
      : unitData.shares_num;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    reserveUnit(
      { ...data, shares_num: finalShareNum, walletunit_id: unitData.id },
      {
        onSuccess: () => {
          setOpen(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] sm:max-w-[500px] bg-white rounded-2xl p-5 sm:p-6 shadow-2xl">
        ...
        <DialogDescription className="text-xl text-center">
          لحجز الحصص واضافتها الي محفظتك العقارية برجاء تسجيل بياناتك
        </DialogDescription>
